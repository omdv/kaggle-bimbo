...
if __name__ == "__main__":
    ...
    train = df[df.WeekNum < 9]
    test = df[df.WeekNum == 9]


    train['log_demand'] = train['AdjDemand'].apply(np.log1p)
    test['log_demand'] = test['AdjDemand'].apply(np.log1p)

    col_lst = [
            ['ProductId', 'ClientId', 'DepotId'],
            ['ProductId', 'RouteId'],
            ]
    f = open('coef.csv', 'a')
