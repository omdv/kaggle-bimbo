...
train=fread('/mnt/gcs-bucket/train.csv',select = c("Semana",'Cliente_ID', 'Producto_ID', 'Agencia_ID', 'Ruta_SAK', 'Demanda_uni_equil'))
test=fread('/mnt/gcs-bucket/test.csv',select = c("Semana",'id','Cliente_ID', 'Producto_ID', 'Agencia_ID', 'Ruta_SAK'))
...
data=merge(data,data1[Semana>8,.(pc_targetl1=mean(target)), by=.(Semana,Cliente_ID,Producto_ID)],all.x=T, by=c("Semana","Cliente_ID","Producto_ID"))

data1<-data[,.(Semana=Semana+1,Producto_ID,Ruta_SAK,target)]
data=merge(data,data1[Semana>8,.(pr_targetl1=mean(target)), by=.(Semana,Producto_ID,Ruta_SAK)],all.x=T, by=c("Semana","Producto_ID","Ruta_SAK"))
...
if(TRUE){
  ...
  data=merge(data,data1[Semana>8,.(pc_targetl2=mean(target)), by=.(Semana,Cliente_ID,Producto_ID)],all.x=T, by=c("Semana","Cliente_ID","Producto_ID"))
  data1<-data[,.(Semana=Semana+3,Cliente_ID,Producto_ID,target)]
  data=merge(data,data1[Semana>8,.(pc_targetl3=mean(target)), by=.(Semana,Cliente_ID,Producto_ID)],all.x=T, by=c("Semana","Cliente_ID","Producto_ID"))
  data1<-data[,.(Semana=Semana+4,Cliente_ID,Producto_ID,target)]
  data=merge(data,data1[Semana>8,.(pc_targetl4=mean(target)), by=.(Semana,Cliente_ID,Producto_ID)],all.x=T, by=c("Semana","Cliente_ID","Producto_ID"))
  data1<-data[,.(Semana=Semana+5,Cliente_ID,Producto_ID,target)]
  data=merge(data,data1[Semana>8,.(pc_targetl5=mean(target)), by=.(Semana,Cliente_ID,Producto_ID)],all.x=T, by=c("Semana","Cliente_ID","Producto_ID"))

  data1<-data[,.(Semana=Semana+2,Ruta_SAK,Producto_ID,target)]
  data=merge(data,data1[Semana>8,.(pr_targetl2=mean(target)), by=.(Semana,Ruta_SAK,Producto_ID)],all.x=T, by=c("Semana","Ruta_SAK","Producto_ID"))
  data1<-data[,.(Semana=Semana+3,Ruta_SAK,Producto_ID,target)]
  data=merge(data,data1[Semana>8,.(pr_targetl3=mean(target)), by=.(Semana,Ruta_SAK,Producto_ID)],all.x=T, by=c("Semana","Ruta_SAK","Producto_ID"))
  data1<-data[,.(Semana=Semana+4,Ruta_SAK,Producto_ID,target)]
  data=merge(data,data1[Semana>8,.(pr_targetl4=mean(target)), by=.(Semana,Ruta_SAK,Producto_ID)],all.x=T, by=c("Semana","Ruta_SAK","Producto_ID"))
  data1<-data[,.(Semana=Semana+5,Ruta_SAK,Producto_ID,target)]
  data=merge(data,data1[Semana>8,.(pr_targetl5=mean(target)), by=.(Semana,Ruta_SAK,Producto_ID)],all.x=T, by=c("Semana","Ruta_SAK","Producto_ID"))
}
...
data_test_lag1=data_test1[,.(Cliente_ID,Producto_ID,Ruta_SAK)]
data_test_lag1$pc_targetl1=res
data_test_lag1$pr_targetl1=res
data_test_lag1pc=data_test_lag1[,.(pc_targetl1=mean(pc_targetl1)), by=.(Cliente_ID,Producto_ID)]
data_test_lag1pr=data_test_lag1[,.(pr_targetl1=mean(pr_targetl1)), by=.(Producto_ID,Ruta_SAK)]


results=data.frame(id=data_test1$id,Demanda_uni_equil=res)

data_test2=data_test[Semana==11,]
data_test2[,pc_targetl1:=NULL]
data_test2[,pr_targetl1:=NULL]

# Merge lagged values of target variable to test the set for the 11th week
data_test2=merge(data_test2,data_test_lag1pc,all.x=T,by=c('Cliente_ID','Producto_ID'))
data_test2=merge(data_test2,data_test_lag1pr,all.x=T,by=c('Ruta_SAK','Producto_ID'))
...
options(digits=18)
write.csv(results,file='/mnt/gcs-bucket/rscript_results.csv',row.names=F)